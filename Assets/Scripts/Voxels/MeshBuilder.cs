    ...
    public class MeshBuilder : NetworkBehaviour
    {
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if (IsServer)
            {
                ...
                InitTemplate();
                SetDimensionsAsServer();
            }
        }

        private void SetDimensionsAsClient()
        {
            SetDimensions();
        }
        ...
        {
            chunkDimensions = GetComponent<Chunk>().chunkDimensions;
        }

        private void SetDimensionsAsServer()
        {
            SetDimensions();
        ...
        private void UpdateMesh(Vector3[] vertices, Vector3[] normals, Vector2[] uvs, int[] indices, int len)
        {
            _mesh.Clear();
            _mesh.SetVertices(vertices);
            _mesh.SetNormals(normals);
            _mesh.SetUVs(0, uvs);
            _mesh.indexFormat = len > ushort.MaxValue ? IndexFormat.UInt32 : IndexFormat.UInt16;
        ...
        [ClientRpc]
        public void SendMeshToClientRpc(Vector3[] vertices, Vector3[] normals, Vector2[] uvs, int[] indices, int len)
        {
            UpdateMesh(vertices, normals, uvs, indices, len);
        }
        ...
        private void RunJobs()
        {
            ...
            if (_jobScheduled)
            {
                ...
                SendMeshToClientRpc(_vertices.ToArray(), _normals.ToArray(), _uv.ToArray(), _indices.ToArray(), _indices.Length);
                _jobScheduled = false;
            }
        ...
        public void AddVoxelServerRpc(int x, int y, int z, byte type)
        {
            //Dependency.Complete();
        ...
        //[ServerRpc]
        //public void AddVoxelServerRpc(int3 coords, byte type)
        //{
        //    AddVoxel(coords.x, coords.y, coords.z, type);
        //}

        //[ServerRpc] can't pass int3[]
        //public void AddVoxelsServerRpc(int3[] coords, byte[] type)
        //{
        //    var count = 0;
