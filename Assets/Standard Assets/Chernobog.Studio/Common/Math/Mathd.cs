...
namespace Chernobog.Studio.Common
{
	...
	public static double ibeta(double aa, double bb, double xx)
	{
		...
		y = a*Math.Log(x);
		t = b*Math.Log(xc);
		if ((a + b) < C.MAXGAM && Math.Abs(y) < C.MAXLOG && Math.Abs(t) < C.MAXLOG)
		{
			t = Math.Pow(xc, b);
			t *= Math.Pow(x, a);
			t /= a;
	...
	private static double pseries(double a, double b, double x)
	{
		...
		u = a*Math.Log(x);
		if ((a + b) < C.MAXGAM && Math.Abs(u) < C.MAXLOG)
		{
			t = gamma(a + b)/(gamma(a)*gamma(b));
			s = s*t*Math.Pow(x, a);
		}
